        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-4">
          ...
          {order.season && (
            <div className="flex items-center gap-1.5 col-span-2">
              ...
              <span className="truncate">{order.season}{order.colorway ? ` · ${order.colorway}` : ""}</span>
            </div>
          )}
        </div>
...
export default function StyleOrders() {
  ...
  async function handleCopy(order: StyleOrderRecord) {
    ...
    try {
      const raw = {
        ...rest,
        styleName: `Copy of ${order.styleName}`,
        orderStatus: "Draft",
      ...
      toast({ title: "Order copied", description: `"Copy of ${order.styleName}" created` });
      const created = result as { data?: { id?: number } };
      if (created?.data?.id) setLocation(`/style-orders/${created.data.id}`);
    } catch {
    ...
    <AppLayout username={user.username} role={user.role} onLogout={handleLogout} isLoggingOut={logoutMutation.isPending}>
      <div className="max-w-screen-xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Style Orders</h1>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input
              type="text"
              placeholder="Search style name, code, client…"
              value={search}
