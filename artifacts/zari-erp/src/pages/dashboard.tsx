...
// ── Card base class (white + gold shadow) ─────────────────────────
const card = [
          ...
          {kpiCards.map((c) => (
            <div key={c.label} className={`${card} fade-up`} style={{ animationDelay: c.delay }}>
              ...
              <div className="p-5">
                ...
                <p className="text-[2.6rem] font-black leading-none tracking-tight text-gray-900">
                  {c.value}
                </p>
          ...
          <div className={`${card} fade-up lg:col-span-2 p-6`} style={{ animationDelay: "320ms" }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full"
                style={{ color: G_DIM, background: `${G}12`, border: `1px solid ${G}30` }}>
                6 Months
              </span>
            </div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={trendData} barCategoryGap="28%" barGap={3}>
                ...
                <Bar dataKey="styleQty"  name="Style Orders"  fill={SLATE} radius={[5,5,0,0]} />
            ...
            <div className="flex justify-center mb-4">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-2xl font-black text-gray-900">{totalStatus}</span>
            ...
            <div className="space-y-2">
              {statusData.map((s) => (
                <div key={s.name} className="flex items-center gap-2.5">
                  ...
                  <div className="w-14 h-1 rounded-full overflow-hidden bg-gray-100">
                    <div className="h-full rounded-full" style={{ width: `${(s.value/totalStatus)*100}%`, background: s.color }} />
                  </div>
                  <span className="text-xs font-bold text-gray-700 w-5 text-right">{s.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={`${card} fade-up lg:col-span-2 p-6`} style={{ animationDelay: "480ms" }}>
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <h3 className="text-sm font-bold text-gray-800">Latest 5 Orders</h3>
              </div>
            ...
            {/* Table header */}
            <div className="grid grid-cols-[1fr_1.4fr_1fr_1fr] gap-3 pb-2.5 mb-1 border-b border-gray-100">
              {["Order", "Client", "Status", "Date"].map((h, i) => (
                <span key={h} className={`text-[9px] font-black uppercase tracking-widest text-gray-400 ${i === 3 ? "text-right" : ""}`}>
                  {h}
                </span>
              ))}
            </div>
            {recentOrders.map((order, idx) => (
              <button key={order.code} onClick={() => setLocation("/style-orders")}
                className="w-full grid grid-cols-[1fr_1.4fr_1fr_1fr] gap-3 py-2.5 text-left rounded-xl px-1 -mx-1 transition-all duration-200 hover:bg-amber-50/60"
                ...
                <span className="text-xs truncate text-gray-500">{order.client}</span>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full w-fit ${statusPill[order.status] ?? "bg-gray-100 text-gray-500"}`}>
                  {order.status}
                </span>
                <span className="text-xs text-right text-gray-400">{order.date}</span>
              </button>
            ))}
          </div>
                        ...
                        return (
                          <td key={i} className="py-2 text-center px-1">
                            <span className="inline-flex items-center justify-center h-8 w-8 rounded-xl font-black text-sm mx-auto transition-all"
                              style={{
                                background: isEmpty ? "#F9FAFB"
                                          : isMax   ? G
                                          : isHigh  ? `${G}28`
                                          :           `${G}12`,
                                color:      isEmpty ? "#D1D5DB"
                                          : isMax   ? "#fff"
                                          : isHigh  ? G_DIM
                                          :           "#B8A240",
