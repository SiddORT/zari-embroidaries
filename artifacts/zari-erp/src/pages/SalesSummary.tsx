...
import {
  ...
  FileX, Truck, BarChart2, CheckCircle,
} from "lucide-react";
...
function KpiCard({
  label, value, sub, Icon, accent = G, highlight = false,
}: {
  label: string; value: string; sub: string; Icon: React.ElementType;
  accent?: string; highlight?: boolean;
}) {
  return (
    <div className={`${card} ${highlight ? "ring-2 ring-[#C6AF4B]/40" : ""}`}>
      <GoldBar />
      <div className="p-5">
...
export default function SalesSummary() {
  ...
  useEffect(() => {
    fetch("/api/clients/all", { headers: { Authorization: `Bearer ${tok()}` } })
      .then(r => r.json())
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/accounts/sales-summary?${p}`, {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-2xl font-black text-gray-900">Sales Summary</h1>
            <p className="text-sm text-gray-400 mt-0.5">Invoiced, received and receivables overview</p>
          </div>
          <div className="h-10 w-10 rounded-2xl flex items-center justify-center"
            style={{ background: `${G}18`, border: `1px solid ${G}35` }}>
            <TrendingUp className="h-5 w-5" style={{ color: G_DIM }} />
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-end mb-8 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={clientId} onChange={e => setClientId(e.target.value)} className={`${INP} min-w-[180px]`}>
          ...
          {(fromDate || toDate || clientId) && (
            <button
              onClick={() => { setFromDate(""); setToDate(""); setClientId(""); }}
        ...
        {loading ? (
          ...
          <>
            {/* Primary KPIs */}
            <SectionLabel text="Revenue Overview" />
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard label="Total Invoiced"      value={fmt(d.totalInvoiced ?? 0)}      sub={`${d.invoiceCount ?? 0} invoices`}    Icon={DollarSign} />
              <KpiCard label="Total Received"      value={fmt(d.totalReceived ?? 0)}      sub="Payments collected"                    Icon={TrendingUp}  accent="#10b981" />
              <KpiCard label="Pending Receivables" value={fmt(d.pendingReceivables ?? 0)} sub="Yet to be collected"                   Icon={Clock}       accent="#f59e0b" />
              <KpiCard label="Overdue Receivables" value={fmt(d.overdueReceivables ?? 0)} sub="Past due date"                        Icon={AlertCircle} accent="#ef4444" />
            </div>

            {/* Breakdown */}
            <SectionLabel text="Sales Breakdown" />
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              <KpiCard label="Credit Notes Issued"     value={fmt(d.totalCreditNotes ?? 0)}  sub={`${d.creditCount ?? 0} notes`}      Icon={FileX}   accent="#6366f1" />
              <KpiCard label="Shipping Recovered"      value={fmt(d.shippingRecovered ?? 0)} sub="Shipping charges billed"            Icon={Truck}   accent="#0ea5e9" />
              <KpiCard label="Net Sales Value"         value={fmt(d.netSalesValue ?? 0)}     sub="Invoiced minus credit notes"        Icon={BarChart2} />
            </div>

            {/* Net Revenue spotlight */}
            ...
            <div className="rounded-2xl overflow-hidden border-2 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              ...
              <div>
                <p className="text-[9px] font-black uppercase tracking-[0.25em] mb-1" style={{ color: G }}>Net Revenue (Sales)</p>
                <p className="text-4xl font-black text-white">{fmt(d.netSalesValue ?? 0)}</p>
                <p className="text-xs mt-1" style={{ color: `${G}BB` }}>Total invoiced minus credit notes issued</p>
              </div>
              <div className="flex flex-col gap-2 text-sm shrink-0">
                <div className="flex items-center gap-2 text-white/70">
                  <span style={{ color: G }}>▲</span>
                  <span>Invoiced: <strong className="text-white">{fmt(d.totalInvoiced ?? 0)}</strong></span>
                </div>
                <div className="flex items-center gap-2 text-white/70">
                  <span className="text-purple-400">▼</span>
                  <span>Credit Notes: <strong className="text-white">{fmt(d.totalCreditNotes ?? 0)}</strong></span>
                </div>
              </div>
            </div>
          </>
