...
export default function StyleCategoryMaster() {
  ...
  const canView = can(MASTERS_STYLE_CATEGORIES.VIEW);
  const canAddEdit = can(MASTERS_STYLE_CATEGORIES.ADD_EDIT);
  const canDelete = can(MASTERS_STYLE_CATEGORIES.DELETE);
  const canExport = can(MASTERS_STYLE_CATEGORIES.DOWNLOAD);
  ...
  return (
    <AppLayout username={user.username} role={user.role} onLogout={handleLogout} isLoggingOut={logoutMutation.isPending}>
      <div className="max-w-screen-xl mx-auto space-y-5">
        <MasterHeader title="Style Category Master" onAdd={openCreate} addLabel="Add Category" addPermission={MASTERS_STYLE_CATEGORIES.ADD_EDIT} />
