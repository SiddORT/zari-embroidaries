...
import {
  useDepartmentMasterList, useCreateDepartmentMaster, useUpdateDepartmentMaster,
  useToggleDepartmentStatus, useDeleteDepartmentMaster, useImportDepartments,
  fetchAllDepartmentsForExport,
  type DepartmentRecord, type DepartmentFormData, type StatusFilter,
} from "@/hooks/useDepartmentMaster";

const EMPTY_FORM: DepartmentFormData = { name: "", isActive: true };
...
const asRec = (r: TableRow) => r as unknown as DepartmentRecord;
...
export default function DepartmentMaster() {
  ...
  const { data, isLoading } = useDepartmentMasterList({ search, status, page, limit });
  const createMutation = useCreateDepartmentMaster();
  const updateMutation = useUpdateDepartmentMaster();
  const toggleStatus = useToggleDepartmentStatus();
  const deleteMutation = useDeleteDepartmentMaster();
  const importMutation = useImportDepartments();
  const { can } = useMyPermissions();
  const canView = can(MASTERS_DEPARTMENTS.VIEW);
  const canAddEdit = can(MASTERS_DEPARTMENTS.ADD_EDIT);
  const canDelete = can(MASTERS_DEPARTMENTS.DELETE);
  const canExport = can(MASTERS_DEPARTMENTS.DOWNLOAD);
  ...
  function openEdit(r: DepartmentRecord) {
    if (!canAddEdit) return;
  ...
  function validateName(val: string): string | undefined {
    ...
    if (trimmed.length > NAME_MAX) return `Department Name must be ${NAME_MAX} characters or fewer.`;
    if (!NAME_REGEX.test(trimmed)) return "Department Name must contain only letters and spaces (max 100 characters).";
    return undefined;
  }
  ...
  async function handleExportAll() {
    ...
    try {
      const allRows = await fetchAllDepartmentsForExport(search, status);
  ...
  function downloadSample() {
    ...
    XLSX.writeFile(wb, "Department_Import_Sample.xlsx");
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const records = jsonRows.map((row) => ({ name: String(row["Department Name"] ?? "").trim() }));
  ...
  const rows: TableRow[] = ((data?.data ?? []) as DepartmentRecord[]).map((r, i) => ({
  ...
  const columns: Column[] = [
    ...
    { key: "name", label: "Department Name", render: (r) => <span className="font-medium text-gray-900">{asRec(r).name}</span> },
    ...
    { key: "createdBy", label: "Created By", render: (r) => <span className="text-gray-500">{asRec(r).createdBy}</span> },
    ...
    { key: "updatedBy", label: "Updated By", render: (r) => <span className="text-gray-500">{asRec(r).updatedBy || "—"}</span> },
    { key: "updatedAt", label: "Updated At", render: (r) => <span className="text-gray-500 whitespace-nowrap">{formatDateTable(asRec(r).updatedAt)}</span> },
    {
  ...
  return (
    <AppLayout username={user.username} role={user.role} onLogout={handleLogout} isLoggingOut={logoutMutation.isPending}>
      <div className="max-w-screen-xl mx-auto space-y-5">
        <MasterHeader title="Department Master" onAdd={openCreate} addLabel="Add Department" addPermission={MASTERS_DEPARTMENTS.ADD_EDIT} />

        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <SearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search departments…" />
          </div>
      ...
      <ConfirmModal open={deleteId !== null} onCancel={() => setDeleteId(null)} onConfirm={() => { void handleDelete(); }}
        title="Delete Department" message="Are you sure you want to delete this department?" loading={deleteMutation.isPending} />
