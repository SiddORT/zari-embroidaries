...
import {
  Pencil, Trash2, Plus, FileDown, FileSpreadsheet, FileUp, ChevronDown, Loader2, Eye,
} from "lucide-react";
...
export default function SwatchMaster() {
  ...
  const canView = can(MASTERS_SWATCHES.VIEW);
  const canAddEdit = can(MASTERS_SWATCHES.ADD_EDIT);
  const canDelete = can(MASTERS_SWATCHES.DELETE);
  const canExport = can(MASTERS_SWATCHES.DOWNLOAD);
        ...
        <div className="flex gap-2">
          {(canView) && (
            <button onClick={() => setLocation(`/masters/swatches/${rec.id}/edit`)}
              className="p-1 rounded hover:bg-gray-100 text-gray-600" title="View"><Eye size={15} /></button>
          )}
          {canAddEdit && (
            <button onClick={() => setLocation(`/masters/swatches/${rec.id}/edit`)}
              className="p-1 rounded hover:bg-gray-100 text-gray-600" title="Edit"><Pencil size={15} /></button>
          )}
          {canDelete && (
            <button onClick={() => setDeleteId(rec.id)}
              className="p-1 rounded hover:bg-red-50 text-red-500" title="Delete"><Trash2 size={15} /></button>
          )}
        </div>
      );
    }},
