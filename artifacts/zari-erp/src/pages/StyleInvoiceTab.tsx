...
import { useStyleInvoice, useNextInvoiceNo, useCreateInvoice, useUpdateInvoice, useDeleteInvoice, type InvoiceLineItem } from "@/hooks/useInvoice";
import {
...
function buildItemsFromStyleCosting(
  bomRows: ReturnType<typeof useStyleBom>["data"],
  pos: ReturnType<typeof useStylePOs>["data"],
  prs: ReturnType<typeof useStylePRs>["data"],
  artisanTimesheets: ReturnType<typeof useStyleArtisanTimesheets>["data"],
  outsourceJobs: ReturnType<typeof useStyleOutsourceJobs>["data"],
  customCharges: ReturnType<typeof useStyleCustomCharges>["data"],
): InvoiceLineItem[] {
...
// ── Main component ────────────────────────────────────────────────────────────
export default function StyleInvoiceTab({
  styleOrderId, orderCode, styleName, clientName: propClientName,
}: {
  styleOrderId: number;
  orderCode?: string;
  styleName?: string;
  ...
  const { data: bomRows = [], isLoading: lb } = useStyleBom(styleOrderId);
  const { data: pos = [], isLoading: lp } = useStylePOs(styleOrderId);
  const { data: prs = [], isLoading: lr } = useStylePRs(styleOrderId);
  const { data: artisanTimesheets = [], isLoading: la } = useStyleArtisanTimesheets(styleOrderId);
  const { data: outsourceJobs = [], isLoading: lo } = useStyleOutsourceJobs(styleOrderId);
  const { data: customCharges = [], isLoading: lc } = useStyleCustomCharges(styleOrderId);
  ...
  useEffect(() => {
    ...
    autoImportedRef.current = true;
    const imported = buildItemsFromStyleCosting(bomRows, pos, prs, artisanTimesheets, outsourceJobs, customCharges);
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const payload = { ...form, styleOrderId, items };
      if (invoice) {
    ...
    <>
      <style>{`
        @media print {
          ...
          #style-invoice-print, #style-invoice-print * { visibility: visible; }
          #style-invoice-print { position: fixed; top: 0; left: 0; width: 100%; font-size: 10px; }
      ...
      <div className="no-print flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-xs text-gray-400">{invoiceNo} · {styleName ?? orderCode}</p>
          </div>
      ...
      {/* ── Invoice Document ─────────────────────────────────────────────── */}
      <div id="style-invoice-print" className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
