...
import {
  useSwatchCategoryMasterList, useCreateSwatchCategoryMaster, useUpdateSwatchCategoryMaster,
  useToggleSwatchCategoryStatus, useDeleteSwatchCategoryMaster, useImportSwatchCategories,
  fetchAllSwatchCategoriesForExport,
  type SwatchCategoryRecord, type SwatchCategoryFormData, type StatusFilter,
} from "@/hooks/useSwatchCategoryMaster";

const EMPTY_FORM: SwatchCategoryFormData = { name: "", isActive: true };
type FormErrors = Partial<Record<keyof SwatchCategoryFormData, string>>;
...
const asCat = (r: TableRow) => r as unknown as SwatchCategoryRecord;

export default function SwatchCategoryMaster() {
  ...
  const [editRecord, setEditRecord] = useState<SwatchCategoryRecord | null>(null);
  const [form, setForm] = useState<SwatchCategoryFormData>(EMPTY_FORM);
  const [errors, setErrors] = useState<FormErrors>({});
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const [toggleTarget, setToggleTarget] = useState<SwatchCategoryRecord | null>(null);
  ...
  const { data, isLoading } = useSwatchCategoryMasterList({ search, status, page, limit });
  const createMutation = useCreateSwatchCategoryMaster();
  const updateMutation = useUpdateSwatchCategoryMaster();
  const toggleStatus = useToggleSwatchCategoryStatus();
  const deleteMutation = useDeleteSwatchCategoryMaster();
  const importMutation = useImportSwatchCategories();

  const { can } = useMyPermissions();
  const canView = can(MASTERS_SWATCH_CATEGORIES.VIEW);
  const canAddEdit = can(MASTERS_SWATCH_CATEGORIES.ADD_EDIT);
  const canDelete = can(MASTERS_SWATCH_CATEGORIES.DELETE);
  const canExport = can(MASTERS_SWATCH_CATEGORIES.DOWNLOAD);
  ...
  function openEdit(r: SwatchCategoryRecord) {
  ...
  async function handleExportAll() {
    ...
    try {
      const allRows = await fetchAllSwatchCategoriesForExport(search, status);
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Swatch Categories");
      XLSX.writeFile(wb, "Swatch_Categories.xlsx");
  ...
  function downloadSample() {
    ...
    const sampleData = [
      ...
      { "Category Name": "Fabric Samples" },
    ];
    const ws = XLSX.utils.json_to_sheet(sampleData);
    ws["!cols"] = [{ wch: 30 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Swatch Category Template");
    XLSX.writeFile(wb, "Swatch_Category_Import_Sample.xlsx");
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const records = jsonRows.map((row) => ({ name: String(row["Category Name"] ?? "").trim() }));
  ...
  const rows: TableRow[] = ((data?.data ?? []) as SwatchCategoryRecord[]).map((r, i) => ({
    ...(r as unknown as TableRow), _srNo: (page - 1) * limit + i + 1,
  }));

  const columns: Column[] = [
    { key: "_srNo", label: "Sr No", className: "w-16 text-center" },
    { key: "name", label: "Category Name", render: (r) => <span className="font-medium text-gray-900">{asCat(r).name}</span> },
    { key: "isActive", label: "Status", render: (r) => <StatusToggle isActive={asCat(r).isActive} onToggle={() => canAddEdit && setToggleTarget(asCat(r))} loading={toggleStatus.isPending && toggleTarget?.id === asCat(r).id} /> },
      ...
      key: "actions", label: "Actions", render: (r) => {
        ...
        return (
          <div className="flex gap-2">
            {canAddEdit && <button onClick={() => openEdit(rec)} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors" title="Edit"><Pencil size={15} /></button>}
            {canDelete && <button onClick={() => setDeleteId(rec.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete"><Trash2 size={15} /></button>}
          </div>
        );
      },
    },
  ...
  return (
    <AppLayout username={user.username} role={user.role} onLogout={handleLogout} isLoggingOut={logoutMutation.isPending}>
      <div className="max-w-screen-xl mx-auto space-y-5">
        <MasterHeader title="Swatch Category Master" onAdd={openCreate} addLabel="Add Category" addPermission={MASTERS_SWATCH_CATEGORIES.ADD_EDIT} />
      ...
      <MasterFormModal open={modalOpen} onClose={() => setModalOpen(false)}
        title={editRecord ? "Edit Swatch Category" : "Add Swatch Category"}
        ...
        <div className="flex flex-col gap-1.5">
          <InputField
            ...
            placeholder="e.g. Thread, Fabric Samples"
            value={form.name}
            maxLength={NAME_MAX}
            onChange={(e) => {
      ...
      <ConfirmModal
        ...
        message={
          toggleTarget
            ? `Are you sure you want to change the status of "${toggleTarget.name}" to ${toggleTarget.isActive ? "Inactive" : "Active"}?`
            : ""
        }
      ...
      <ConfirmModal open={deleteId !== null} onCancel={() => setDeleteId(null)} onConfirm={() => { void handleDelete(); }}
        title="Delete Category" message="Are you sure you want to delete this swatch category?" loading={deleteMutation.isPending} />

      <ImportResultModal
        open={importResultOpen}
        result={importResult}
        entityName="Swatch Categories"
