...
import {
  ...
  useToggleSwatchCategoryStatus, useDeleteSwatchCategoryMaster,
...
export default function SwatchCategoryMaster() {
  ...
  const rows: TableRow[] = ((data?.data ?? []) as SwatchCategoryRecord[]).map((r, i) => ({ ...(r as unknown as TableRow), _srNo: (page - 1) * limit + i + 1 }));

  const columns: Column[] = [
    { key: "_srNo", label: "Sr No" },
    { key: "name", label: "Category Name", render: (r) => asCat(r).name },
    ...
    <AppLayout username={user.username} role={user.role} onLogout={handleLogout} isLoggingOut={logoutMutation.isPending}>
      <div className="max-w-screen-xl mx-auto space-y-5">
        ...
        <div className="flex gap-3 items-center">
          ...
          <ExportExcelButton data={(data?.data ?? []) as unknown as Record<string, unknown>[]} columns={exportColumns} filename="swatch-categories" />
        </div>
        ...
        <MasterFormModal open={modalOpen} onClose={() => setModalOpen(false)}
          ...
          <InputField label="Category Name" value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))}
            error={errors.name} required placeholder="e.g. Thread, Embroidery" />
        ...
        <ConfirmModal open={deleteId !== null} onCancel={() => setDeleteId(null)} onConfirm={() => { void handleDelete(); }}
          title="Delete Category" message="Are you sure you want to delete this swatch category?" />
      </div>
    </AppLayout>
  );
}
